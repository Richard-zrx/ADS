...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare HuggingFaceH4/MATH-500 into unified VeRL/GRPO format.")
...
def main() -> None:
    ...
    output_dir = output_root / "math500"
    parquet_path = output_dir / "test.parquet"
    example_path = output_dir / "example.json"
    output_dir.mkdir(parents=True, exist_ok=True)

    source_ds = datasets.load_dataset(DATASET_NAME, split="test", cache_dir=args.cache_dir)
    ...
    for idx in range(input_rows):
        ...
        source_id = to_text(example.get("unique_id")).strip() or str(idx)
        subject = to_optional_text(example.get("subject"))
        level = to_optional_int(example.get("level"))
        record = {
            "data_source": "math500",
    ...
    print(f"[math500] input_rows={input_rows} output_rows={output_rows}")
    print(f"[math500] parquet={parquet_path}")
    print(f"[math500] example={example_path}")
