...
def validate_config(
    ...
    # check eval config
    if config.actor_rollout_ref.rollout.val_kwargs.n <= 0:
        raise ValueError("actor_rollout_ref.rollout.val_kwargs.n must be a positive integer.")

    n_per_data_source = config.actor_rollout_ref.rollout.val_kwargs.get("n_per_data_source", {})
    if n_per_data_source is None:
        n_per_data_source = {}
    if not isinstance(n_per_data_source, DictConfig | dict):
        raise ValueError(
            "actor_rollout_ref.rollout.val_kwargs.n_per_data_source must be a dict mapping data_source to n."
        )
    for data_source, repeat_n in n_per_data_source.items():
        if not isinstance(data_source, str) or not data_source:
            raise ValueError(
                "actor_rollout_ref.rollout.val_kwargs.n_per_data_source keys must be non-empty strings."
            )
        if isinstance(repeat_n, bool) or not isinstance(repeat_n, int) or repeat_n <= 0:
            raise ValueError(
    ...
    # check validation response length override for vLLM
    if config.actor_rollout_ref.rollout.name == "vllm":
        val_response_length = config.actor_rollout_ref.rollout.get("val_response_length", None)
        if val_response_length is not None:
            max_prompt_length = config.data.get("max_prompt_length", None)
            if max_prompt_length is None:
                raise ValueError(
                    "data.max_prompt_length must be set when actor_rollout_ref.rollout.val_response_length is used."
                )
            required_max_model_len = max_prompt_length + val_response_length
            max_model_len = config.actor_rollout_ref.rollout.get("max_model_len", None)
            if max_model_len is None:
                raise ValueError(
                    "actor_rollout_ref.rollout.max_model_len must be set when using "
                    "actor_rollout_ref.rollout.val_response_length. "
                    f"Expected max_model_len >= data.max_prompt_length + val_response_length "
                    f"(>= {required_max_model_len})."
                )
            if max_model_len < required_max_model_len:
                raise ValueError(
                    "actor_rollout_ref.rollout.max_model_len is too small for validation response override. "
                    f"Got max_model_len={max_model_len}, "
                    f"but require >= data.max_prompt_length + val_response_length "
                    f"(>= {required_max_model_len})."
                )
