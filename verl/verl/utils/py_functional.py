...
TIMEOUT_TERMINATE_GRACE_SECONDS = float(os.getenv("VERL_TIMEOUT_TERMINATE_GRACE_SECONDS", "0.5"))
TIMEOUT_KILL_GRACE_SECONDS = float(os.getenv("VERL_TIMEOUT_KILL_GRACE_SECONDS", "0.2"))
TIMEOUT_VERBOSE_WARNINGS = os.getenv("VERL_TIMEOUT_VERBOSE_WARNINGS", "0") == "1"
    ...
    def decorator(func):
        if use_signals:
            ...
            def wrapper_mp(*args, **kwargs):
                ...
                if process.is_alive():
                    process.terminate()
                    process.join(timeout=TIMEOUT_TERMINATE_GRACE_SECONDS)
                    if process.is_alive():
                        # Escalate to force kill to prevent zombie timeout workers.
                        if hasattr(process, "kill"):
                            process.kill()
                            process.join(timeout=TIMEOUT_KILL_GRACE_SECONDS)
                    if process.is_alive() and TIMEOUT_VERBOSE_WARNINGS:
