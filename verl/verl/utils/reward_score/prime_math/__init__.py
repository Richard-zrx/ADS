...
TUPLE_CHARS = "()[]"
SYMPY_TIMEOUT_SECONDS = float(os.getenv("PRIME_MATH_SYMPY_TIMEOUT_SECONDS", "15"))
SYMPY_RETRY_TIMEOUT_SECONDS = float(os.getenv("PRIME_MATH_SYMPY_RETRY_TIMEOUT_SECONDS", "30"))
PRIME_MATH_VERBOSE_ERRORS = os.getenv("PRIME_MATH_VERBOSE_ERRORS", "0") == "1"
...
def _are_equal_under_sympy_impl(ground_truth_normalized: str, given_normalized: str):
    are_equal = False
    try:
...
@timeout_limit(seconds=SYMPY_TIMEOUT_SECONDS)
def are_equal_under_sympy(ground_truth_normalized: str, given_normalized: str):
    return _are_equal_under_sympy_impl(ground_truth_normalized, given_normalized)


@timeout_limit(seconds=SYMPY_RETRY_TIMEOUT_SECONDS)
def are_equal_under_sympy_retry(ground_truth_normalized: str, given_normalized: str):
    return _are_equal_under_sympy_impl(ground_truth_normalized, given_normalized)
...
def grade_answer(given_answer: str, ground_truth: str) -> bool:
    ...
    if (
        ...
        for ground_truth_elem, given_elem in zip(ground_truth_elems, given_elems, strict=True):
            gt_is_int = _str_is_int(ground_truth_elem)
            given_is_int = _str_is_int(given_elem)
            if gt_is_int and given_is_int:
                # Avoid unnecessary sympy for integer equality.
                is_correct = _str_to_int(ground_truth_elem) == _str_to_int(given_elem)
            elif _is_frac(ground_truth_elem) and _is_frac(given_elem):
                # if fractions aren't reduced, then shouldn't be marked as correct
                # so, we don't want to allow sympy.simplify in this case
                is_correct = ground_truth_elem == given_elem
            elif gt_is_int != given_is_int:
                ...
                try:
                    ...
                    # Retry once with a larger timeout to reduce false negatives on heavy symbolic forms.
                    try:
                        is_correct = are_equal_under_sympy_retry(ground_truth_elem, given_elem)
                    except Exception as e:
                        is_correct = False
                        if PRIME_MATH_VERBOSE_ERRORS:
                            print(f"Error: {e} from are_equal_under_sympy_retry, {ground_truth_elem}, {given_elem}")
                except Exception as e:
