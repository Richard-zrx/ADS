...
class ServerAdapter(BaseRollout):
    ...
    def __init__(
        ...
        # Prefix for per-transfer IPC socket path. A new unique socket is created
        # for each weight update to avoid collisions across retries/concurrency.
        self.zmq_handle_prefix = f"ipc:///tmp/rl-colocate-zmq-{self.device_uuid}"
    ...
    async def update_weights(
        ...
        zmq_handle = f"{self.zmq_handle_prefix}-{uuid.uuid4().hex[:10]}.sock"

        future = await self._execute_method(
            "update_weights_from_ipc",
            non_block=True,
            kwargs={**kwargs, "use_shm": self.use_shm, "zmq_handle": zmq_handle},
        )
